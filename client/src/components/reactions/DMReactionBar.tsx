...
import { toggleDMReaction } from '../../services/dm-reaction.service'

const QUICK_EMOJIS = ['👍', '❤️', '😂', '🔥', '👀', '🎉', '💯', '⚔️']

interface DMReactionBarProps {
  dmMessageId: string
  userId: string
...
export default function DMReactionBar({ dmMessageId, userId, reactions, onReactionChange, isAuthor = true }: DMReactionBarProps) {
  ...
  const handleToggle = async (emoji: string) => {
    setShowPicker(false)
    await toggleDMReaction(dmMessageId, userId, emoji)
    onReactionChange()
  }
    ...
    <div className={`flex items-center gap-1 flex-wrap mt-1 ${!isAuthor ? 'flex-row-reverse' : ''}`}>
      ...
      <div className="relative">
        <button
          ...
          className="p-1 rounded-full text-samurai-red hover:text-samurai-red-dark hover:bg-samurai-grey-darker transition-colors"
          title="Add reaction"
        >
